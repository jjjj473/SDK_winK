...
The SDK exposes many helpers for common Wine tasks (over three hundred fifty in total). Some examples include:
...
- Placeholder prefix helpers like `wink_prefix1()` through `wink_prefix100()`
  demonstrate custom winetricks verbs.
- Extra helper placeholders `wink_extra_tool1()` through `wink_extra_tool50()`
  provide even more winetricks wrappers.
